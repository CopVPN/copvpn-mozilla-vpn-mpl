...
bool WindowsRouteMonitor::addExclusionRoute(const IPAddress& prefix) {
  ...
  if (prefix.address().protocol() == QAbstractSocket::IPv6Protocol) {
    ...
    data->DestinationPrefix.PrefixLength = prefix.prefix();
  } else {
  ...
  logger.debug() << "Exclusion route added for:"
...
void WindowsRouteMonitor::flushRouteTable(QHash<IPAddress, MIB_IPFORWARD_ROW2*>& table) {
