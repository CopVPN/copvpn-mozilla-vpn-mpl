...
bool IPUtilsMacos::addInterfaceIPs(const VpnConfig& config) {
    return addIP4AddressToDevice(config) && addIP6AddressToDevice(config);
}

bool IPUtilsMacos::setMTUAndUp(const VpnConfig& config) {
    Q_UNUSED(config);
...
bool IPUtilsMacos::addIP4AddressToDevice(const VpnConfig& config) {
    Q_UNUSED(config);
    QString ifname = MacOSDaemon::instance()->m_wgutils->interfaceName();
...
bool IPUtilsMacos::addIP6AddressToDevice(const VpnConfig& config) {
    Q_UNUSED(config);
    QString ifname = MacOSDaemon::instance()->m_wgutils->interfaceName();
    ...
    if (ret) {
        logger.error() << "Failed to set IPv6: " << deviceAddr << "error:" << strerror(errno);
        return false;
    }
    return true;
}
