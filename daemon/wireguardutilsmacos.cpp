...
bool WireguardUtilsMacos::addInterface(const VpnConfig& config) {
    Q_UNUSED(config);
...
bool WireguardUtilsMacos::updatePeer(const VpnConfig& config) {
...
bool WireguardUtilsMacos::deletePeer(const VpnConfig &config) {
    QByteArray publicKey =
        QByteArray::fromBase64(qPrintable(config.m_serverPublicKey));

    // Clear exclustion routes for this peer.
    deleteExclusionRoute(IPAddress(config.m_serverIpv4AddrIn));
    //deleteExclusionRoute(IPAddress(config.m_serverIpv6AddrIn));
...
bool WireguardUtilsMacos::updateRoutePrefix(const IPAddress& prefix) {
    if (!m_rtmonitor) {
        logger.warning() << "m_rtmoniter isn't initailez pointer";
        return false;
    }
    if (prefix.prefix() > 0) {
        logger.warning() << "prefix > 0";
...
QString WireguardUtilsMacos::waitForTunnelName(const QString& filename) {
    ...
    while ((m_tunnel.state() == QProcess::Running) && timeout.isActive()) {
        ...
        logger.debug() << "connecting to sockname=" << sockName;
        sock.connectToServer(sockName, QIODevice::ReadWrite);
        if (sock.waitForConnected(100)) {
            logger.warning() << "socket connected, returning ifname";
            return ifname;
        }
    }
